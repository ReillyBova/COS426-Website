...
available: true
# Submission url
submitURL: "https://tigerfile.cs.princeton.edu/COS426_S2021/Assignment-2/"
# Gallery url
galleryID: ""
# Feedback url
feedbackURL: ""
...
For full credit, you will need to be extremely careful when updating vertices on the border of a selection. For these vertices, use the boundary weights presented in lecture (which approximate a spline). Note that after subdivision, adjacent even and odd vertices along the border will share non-triangular faces that lie outside the boundary (hereafter referred to as boundary faces), however, they might not all agree on the plane of the face. Therefore, it is necessary to triangulate these boundary faces. For simplicity and for the sake of symmetry, it is easiest group these faces into the call to triangle topology that is made for level of Loop subdivision.
...
For full credit, you will again need to be extremely careful when updating vertices on the border of a selection. For these vertices, use the boundary weights presented in lecture (which approximate a spline; they are used both for Catmull-Clark and Loop). Note that after subdivision, adjacent even and odd vertices (i.e. edge vertices) along the border will share non-triangular boundary faces, however, they might not all agree on the plane of the face. Therefore, it is necessary to triangulate these boundary faces. This process is more complex than Loop because it is not possible group these boundary faces into a call to triangle topology as Catmull-Clark uses quad topology. Instead, we recommend looping through all the boundary faces at the end of each level, and applying a modified triangle topology subdivision to these faces, where odd vertices (created by quad topology) are treated as pre-divided edge midpoints for the triangulation.
